...
class LinkCheckMiddleware(BaseMiddleware):
    ...
    @staticmethod
    async def _check_links_and_handle(user_id: int, obj: types.CallbackQuery | types.Message) -> NoReturn:
        ...
        text, markup = (
            _("Вы подписались не на все каналы! Чтобы продолжить пользоваться ботом, подпишитесь! Ссылки ниже: "),
            await necessary_links_keyboard(
                telegram_id=user_id,
                links_db=links_db,
            ),
        )
        if len(subscribed_links) != len(links_db) and obj.from_user.id not in load_config().tg_bot.admin_ids:
            try:
