...
class YooMoneyWallet:
    ...
    async def get_operation_details(self, operation_id: str) -> OperationDetails:
        ...
        response, data = await send_request(url, headers=self.__headers, data={"operation_id": operation_id})
    ...
    async def create_payment_form(
        ...
        return PaymentForm(link_for_customer=str(response.url), payment_label=unique_label)
